...
//   rest of the nodes in a graph using Dijkstra's algorithm and Floyd Warshall algorithm
...
void leeArchivosDijkstra(string archivo)
{
...
void leeArchivosFloyd(string archivo)
{
    ifstream inputFile(archivo); // Open the file
    if (!inputFile) {
        cerr << "Error: File not found or cannot be opened." << endl;
...
void FloydAlgorythm(){
    ...
    int numNodos = graphFloyd.size(); //number of nodes in the graph
    ...
    for(int k = 0; k < numNodos; k++){ //for each row
        for(int i = 0; i < numNodos; i++){ //for each column
            for(int j = 0; j < numNodos; j++){ //for each node
                if (graphFloyd[i][k] != -1 && graphFloyd[k][j] != -1){ //if the distance between the nodes is not -1 (infinite)
                    currentVal = graphFloyd[i][j];
                    newVal = graphFloyd[i][k] + graphFloyd[k][j];
                    if (currentVal <= -1){ //if the current value is -1 (infinite) then replace it with the new value
                        graphFloyd[i][j] = newVal;
                    }
                    else{ //if the current value is not -1 (infinite) then replace it with the minimum between the current value and the new value
                        graphFloyd[i][j] = min(currentVal, newVal);
                    }
                }
            }
        }
...
int main()
{
    leeArchivosDijkstra("./graph.txt");
    helper_DijkstraAlgroythm();
    leeArchivosFloyd("./graph.txt");
    FloydAlgorythm();
